    ...
    with get_session() as session:
        for attr_type, attr_value in attributes.items():
            ...
            session.run(
                ...
                MERGE (p)-[r:HAS_ATTRIBUTE {fir: $fir_number, attr: $attr_type}]->(a:Entity {text: $value, type: $attr_type, person: $name, fir: $fir_number})
                ON CREATE SET r.source = $filename
                """,
    ...
    with get_session() as session:
        ...
        if complainant_name:
            session.run(
                ...
                MATCH (c:Case {fir_number: $fir_number})
                MERGE (p:Entity {text: $name, type: 'person'})
                MERGE (p)-[:COMPLAINANT_IN]->(c)
                """,
                fir_number=fir_number, name=complainant_name
            )
